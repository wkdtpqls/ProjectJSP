...
<%
	...
	count = pdao.getCategoryCount(coffee);
	...
	String sword = request.getParameter("sword");
	   if (sword==null || sword.equals("")) {
	      ...
	      if(count > 0) {
	         alist = pdao.getArticles(coffee, startRow, pageSize);
	      }
	   } else {
	      count = pdao.getArticleCount("%"+sword+"%",1);   
	      if(count > 0) {
	         alist = pdao.getArticles(startRow, pageSize, "%"+sword+"%",1);
	      }else{
%>
		<script>
			alert("검색결과가 없습니다!");
	...
	<div class="text">
		...
		<div class="txt_box">
		    <ul class="menu">
		        ...
		        <li class="menu-item"><a href="coffeelist.jsp" class="menu-link"  style="background-color:#000040; color:white;border:1px solid #000040;">음료</a></li>
		        <li class="menu-item"><a href="dessertlist.jsp" class="menu-link">디저트</a></li>
		    </ul>
		</div> 
	</div>
</div>
...
<form name="search_form" method="get" action="coffeelist.jsp">
...
//------------------페이지 번호 목록
	if(count > 0) {
		...
		if(startPage > 10) {
%>
			<a href="coffeelist.jsp?pageNum=<%=startPage-10%>">[이전]</a>
<%			
		}
		
		for(int i=startPage; i<=endPage; i++) {
%>
			<a href="coffeelist.jsp?pageNum=<%=i%>"><%= i %></a>
<%			
		}
		
		if(endPage < pageCount) {
%>
			<a href="coffeelist.jsp?pageNum=<%=startPage+10%>">[다음]</a>
<%			
		}		
	}
//------------------
